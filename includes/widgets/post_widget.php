 ...
 */
add_action( 'widgets_init', 'fashy_category_select_post_widget' );
/* Function that registers our widget. */
function fashy_category_select_post_widget() {
...
class fashy_category_select_posts extends WP_Widget {
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'category_select_posts-widget', esc_attr__('Fashy - Premium Posts','fashy'), $widget_ops, '' );
	}
		...
		<div class="widgett <?php if ( has_post_format( 'quote' , get_the_id())) echo 'quote-widget';?> <?php if ( has_post_format( 'link' , get_the_id())) echo 'link-widget';?>">		
			...
			<?php if ( has_post_format( 'video' , get_the_id())) {?>
				...
				<?php  
					if(!empty($postmeta["video_post_url"][0])) {
						...
						echo $embed_code ; ?>
					<div class="wttitle"><h4><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','fashy')?> <?php the_title(); ?>"><?php the_title(); ?></a></h4></div>
			...
			<div class="link-category">
				<div class="blogpostcategory">
					<div class="topBlog">	
						<h2 class="title"><a href="<?php echo esc_url($link) ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','fashy')?> <?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
					</div>			
					<?php if(fashy_getImage(get_the_id(), 'fashy_blog') != '') { ?>	
					...
					<div class="blogimage">		
						<a href="<?php echo esc_url($link) ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','fashy')?> <?php the_title_attribute(); ?>"><?php echo fashy_getImage(get_the_id(), 'fashy_blog'); ?></a>
					</div>
					<?php } ?>												
				</div>
			</div>
			...
			if ( !get_post_format() || has_post_format( 'gallery' , get_the_id())) {
				...
				if ( has_post_thumbnail() ){
					$image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()),'fashy-widget');
					$image = $image[0];}	
					?>			

					<div class="imgholder">
						<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','fashy')?> <?php the_title(); ?>">
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo esc_attr('Title:','fashy') ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
		</p>
		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php echo esc_attr('Number of posts to show:','fashy') ?></label>
			...
			<br /><small><?php echo esc_attr('(at most 10)','fashy') ?></small>
		</p>
		<p>
		...
		<label for="<?php echo esc_attr( $this->get_field_id( 'excerpt' ) ); ?>"><?php echo esc_attr('Show excpert?','fashy') ?></label>
		</p>	
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'n_excerpt' )); ?>"><?php echo esc_attr('Number of words to show in excpert:','fashy') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'category' )); ?>"><?php echo esc_attr('Category to display','fashy') ?></label>
			<?php $args = array(
			...
			<br /><small><?php echo esc_attr('(select category to display)','fashy') ?></small>
		</p>
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'post' )); ?>"><?php echo esc_attr('Post to display','fashy') ?></label>
		<select name="<?php echo esc_attr($this->get_field_name( 'post' )); ?>" id="<?php echo esc_attr($this->get_field_id( 'post' )); ?>"> <option <?php selected( $instance['post'], 'none'); ?>value="none"><?php echo esc_attr('Display more then one post','fashy') ?></option><?php global $post; $args = array( 'numberposts' => -1); $posts = get_posts($args); foreach( $posts as $post ) : setup_postdata($post); ?><option <?php selected( $instance['post'], $post->ID); ?>value="<?php echo esc_attr($post->ID); ?>"><?php the_title(); ?></option> <?php endforeach; ?> </select> 		
		</p>
		<p>
		<br /><small><?php echo esc_attr('Note: Quote post will not be displayed.','fashy') ?></small>	
		<br />
		</p>	
		<?php
	}
