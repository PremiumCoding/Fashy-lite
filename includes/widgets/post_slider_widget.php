 ...
 */
add_action( 'widgets_init', 'fashy_category_select_slider_post_widget' );

/* Function that registers our widget. */
function fashy_category_select_slider_post_widget() {
...
class fashy_category_select_slider_posts extends WP_Widget {
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'category_select_slider_posts-widget', esc_attr__('Fashy - Premium Posts with slider','fashy'), $widget_ops, '' );
	}
			...
			<li>
				<div class="widgett">		
					...
					if ( has_post_thumbnail() ){
						$image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'fashy_widget', false);
						$image = $image[0];}	
					?>			
					<div class="imgholder">
						<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','fashy')?> <?php the_title(); ?>">
							<?php if (has_post_thumbnail( get_the_ID() )) echo fashy_getImage(get_the_id(),'fashy-widget') ;?>		
						</a>
					</div>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Fashy Post slider', 'number' => 3, 'slider' => 2, 'excerpt' => 0, 'n_excerpt' => 10,'category' =>'');
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_attr_e('Number of posts to show:','fashy') ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id( 'excerpt' ) ); ?>"><?php esc_attr_e('Show excpert?','fashy') ?></label>
		</p>	
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'n_excerpt' )); ?>"><?php esc_attr_e('Number of words to show in excpert:','fashy') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'slider' )); ?>"><?php esc_attr_e('Number of posts to show in slider:','fashy') ?></label>
			...
			<br /><small><?php esc_attr_e('(at most 4 for fullwidth slider and 2 for smaller slider)','fashy') ?></small>
		</p>		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'category' )); ?>"><?php esc_attr_e('Category to display','fashy') ?></label>
			<?php $args = array(
			...
			<br /><small><?php esc_attr_e('(select category to display)','fashy') ?></small>
		</p>
		</p>
		<br /><small><?php esc_attr_e('Note: Quote post will not be displayed.','fashy') ?></small>	
		<br />
		</p>	
		<?php
	}
